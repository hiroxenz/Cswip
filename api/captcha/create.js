...
<div id="captcha-slider" style="width:100%;max-width:320px;height:50px;background:#eee;position:relative;border-radius:8px;touch-action:none;">
  <div id="knob-${captcha_id}" style="width:50px;height:50px;background:#333;position:absolute;left:0;top:0;cursor:pointer;border-radius:8px;transition:left 0s linear;"></div>
</div>
...
(function(){
  ...
  let movement = [], dragging = false, startX = 0;
  ...
  function startDrag(e){
    dragging = true;
    startX = getClientX(e) - (parseFloat(knob.style.left) || 0);
    movement = []; // Reset movement on new drag
    e.preventDefault();
  }
  function drag(e){
    if(!dragging) return;
